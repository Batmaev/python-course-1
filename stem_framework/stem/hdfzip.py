...
def read_zip_print_hdf5(input_path, output_path, header_size=24, size_of_innermost_array=1024, dtype='float32', dataset_name='_'):
    with zipfile.ZipFile(input_path) as input_file:
        ...
        N_channels = len(input_file.namelist())
        ...
        with h5py.File(output_path, 'w') as outpit_file:
            dataset = outpit_file.create_dataset(
                dataset_name, 
                (N_entries, N_channels, size_of_innermost_array),
                dtype,
            )
